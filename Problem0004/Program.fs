...
let main argv =
    ...
    let reverseNumber n =
        // recurring function to create new number to flip the existing the number digit one at a time
        let rec loop newNum = function
            // if the new number is 0
            // return the new created number as it is
            |0 -> newNum
            // if number is not 0
            // recall the function with new number * 10 and adding modules of number dividing by 10.
    ...
    let isPalindrome = function
        // if number is equal to reversed number then its palindrome, return truw
        | x  when x = reverseNumber x -> true
        // else return false
        | _ -> false

    // function to get the max palindrome number of multiplication of two numbers between the limits
    let maxPalindromeNumber lowerLimit upperLimit =
        // create the seq of all the palidrome numbers create by multiple two numbers between the limits
        let numbers = seq {
            ...
            for i = lowerLimit to upperLimit do
                ...
                for j = lowerLimit to upperLimit do
                    ...
                    if isPalindrome z then
                        // if yes, return the number to seq else move back to new numbers in the loop
                        yield z
        }
    ...
    // start the stopwatch
    sw.Start() 
    // call function to get max palindrome number product of two numbers between 100 and 900
